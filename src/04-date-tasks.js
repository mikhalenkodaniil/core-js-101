...
function isLeapYear(date) {
  ...
  return !(new Date(date).getFullYear() % 4);
}
...
function angleBetweenClockHands(date) {
  ...
  const hourHandSpeed = 360 / (12 * 60);
  const hourPos = (hour * 60 + min) * hourHandSpeed;
  const minHandSpeed = 360 / 60;
  const minPos = min * minHandSpeed;
  let angle = Math.abs(hourPos - minPos);
